...
revoke_task: asyncio.Task | None = None
    ...
    if target_text and message.channel.id == counting_channel_id:
        if message.content.strip().lower() == target_text:
            await message.channel.send(
                f"🎉 {message.author.mention} typed the correct answer `{target_text}` and won!"
            )
            ...
            if invite.created_at:
                ...
                if age >= MAX_AGE_MINUTES:
                    try:
                        await invite.delete()
                        print(
                            f"❌ Deleted invite {invite.code} (Age: {int(age)} mins)"
                        )
                    except Exception as e:
...
@tree.command(
    name="startrevoke",
    description="Start auto-invite revoking",
    guild=discord.Object(id=GUILD_ID),
)
async def startrevoke(interaction: discord.Interaction):
    ...
    if revoke_task and not revoke_task.done():
        await interaction.response.send_message(
            "🚫 Invite revoking is already running.", ephemeral=True
        )
        return
...
@tree.command(
    name="stoprevoke",
    description="Stop auto-invite revoking",
    guild=discord.Object(id=GUILD_ID),
)
...
@tree.command(
    name="reactsync",
    description="React to recent messages",
    guild=discord.Object(id=GUILD_ID),
)
...
async def settarget(ctx: commands.Context, target: str, channel: discord.TextChannel):
    global target_text, counting_channel_id
    target_text = target.lower()
    counting_channel_id = channel.id
    await ctx.send(
        f"🎯 Target set to `{target}` in {channel.mention}. First one to type it wins!"
    )
